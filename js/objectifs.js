...
function hexSvg(pct, id, size, aurora) {
  const w = size || 80, h = Math.round(w * 1.12);
  const known = pct != null;
  const p = known ? clampPct(pct) : 0;
  const deg = p * 3.6;
  // Variante « Aurora Bento » : arc dégradé indigo→cyan sur la portion atteinte,
  // piste indigo très claire pour le reste, cœur blanc, texte indigo.
  if (aurora) {
    const arc = known
      ? `conic-gradient(from -90deg,#6366f1 0deg,#22d3ee ${deg}deg,#eef2ff ${deg}deg 360deg)`
      : `conic-gradient(#eef2ff 0 360deg)`;
    return `<div class="ob-hex" id="ring-${id}" role="img" aria-label="Progression ${known ? p + ' %' : 'non mesurée'}"
      style="${HEX_STYLE};width:${w}px;height:${h}px;background:${arc};display:flex;align-items:center;justify-content:center;flex-shrink:0">
      <div class="ob-hex" style="${HEX_STYLE};width:${w - 8}px;height:${h - 8}px;background:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center">
        <div id="ringtxt-${id}" style="font-size:${Math.round(w / 4.4)}px;font-weight:900;color:${known ? '#4f46e5' : '#94a3b8'};font-variant-numeric:tabular-nums;line-height:1">${known ? p + '%' : '—'}</div>
...
function renderOverview(residents, tpl) {
  ...
  const cards = residents.map(r => {
    ...
    const hairColor = global != null ? pctColor(global) : '#c7c5e0';
  ...
  return `<div style="font-size:.8rem;color:var(--muted);margin-bottom:.85rem">Cliquez sur un résident pour définir ses axes de travail et suivre sa progression.</div>
    <div class="ob-aurora"><div class="ob-res-grid">${cards}</div></div>`;
}
