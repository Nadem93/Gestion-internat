...
-- ══════════════════════════════════════════════════════════════════════════
-- CENTRE D'ALERTES — prise en charge « je m'en occupe »
-- ──────────────────────────────────────────────────────────────────────────
-- Les alertes sont calculées à la volée (id = « type::ref », donc TEXT). Cette
-- table trace qui prend en charge quelle alerte, pour que l'équipe sache qui
-- traite quoi. Une prise par alerte et par établissement (clé primaire).
-- Tant que la table n'existe pas, l'app se dégrade : la prise en charge est
-- simplement indisponible, le reste du centre d'alertes fonctionne.
-- ══════════════════════════════════════════════════════════════════════════

create table if not exists public.alertes_prises (
  alerte_id        text        not null,
  etablissement_id text        not null,
  pris_par         text,
  pris_par_nom     text,
  pris_le          timestamptz not null default now(),
  primary key (etablissement_id, alerte_id)
);

alter table public.alertes_prises enable row level security;

drop policy if exists alertes_prises_select on public.alertes_prises;
create policy alertes_prises_select
  on public.alertes_prises for select
  using (etablissement_id = (select p.etablissement_id from public.profiles p where p.id = auth.uid()));

drop policy if exists alertes_prises_insert on public.alertes_prises;
create policy alertes_prises_insert
  on public.alertes_prises for insert
  with check (etablissement_id = (select p.etablissement_id from public.profiles p where p.id = auth.uid()));

drop policy if exists alertes_prises_delete on public.alertes_prises;
create policy alertes_prises_delete
  on public.alertes_prises for delete
  using (etablissement_id = (select p.etablissement_id from public.profiles p where p.id = auth.uid()));
